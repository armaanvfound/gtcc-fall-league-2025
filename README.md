...
dashboard - our form, an opponent, a phase, the league - using **DeepSeek with reasoning on** (v4-flash by default — about 3,000
...
reconciled against the scorecards. The prompt (kept in `facts.json` as `_prompt`,
so there is one copy) tells the model to answer only from that file, quote
figures rather than derive them, repeat the sample-size caveats, refuse
off-topic questions, and say plainly when the dashboard does not hold something.
The arithmetic happened in Python and was checked; the model only has to find a
number and phrase it.

**One key, shared by the team.** A small Cloudflare Worker in `worker/` holds the
DeepSeek key as a secret. Teammates open the dashboard and type a shared
passphrase once - no accounts, no keys, nothing to install. The key is never in
this repository, never in the published pages, and never in anyone's browser.

Setup is about five minutes and is written up in **`worker/README.md`**. Until
`ASK_PROXY` in `tools/build.py` points at the deployed Worker, the panel shows
setup notes instead of a chat box.

The proxy is deliberately narrow: it builds the system prompt itself and pins the
model, reasoning effort and token ceiling, so a caller can only ever ask
dashboard questions. It also enforces an origin allow-list, the passphrase, and -
with the optional KV namespace - a per-IP rate limit and a hard daily cap.

`facts.json` refreshes on every `python3 tools/build.py` and the proxy re-reads it
every five minutes, so **adding a match updates the assistant on its own**.
